...
def route_detail(request):
    ...
    events_byduration = sorted(events,key=attrgetter('duration'))
    map(lambda (i,e): setattr(e,'n',i+1), enumerate(events_byduration))
        ...
        #map(lambda e: setattr(e,'ratio',e.duration / event_pb.duration),events)
...
def get_events_response(request,event_add_form=None,event_edit_form=None):
    ...
    dayofyear = int(today.strftime('%j'))
    ...
    if startmonth_prev > startmonth:
        startmonth_prev = startmonth_prev.replace(year=startmonth_prev.year-1)
    ...
    last_7days_events = Event.objects.filter(date__gt=start7days)\
                  .filter(date__lte=today)\
                  .all()
    
    last_30days_events = Event.objects.filter(date__gt=start30days)\
                  .filter(date__lte=today)\
                  .all()
    ...
    summary_dict['this_month_projected'] = dict((k,v/dayofmonth*30) for (k,v) in summary_dict['this_month'].items())
    ...
    summary_dict['this_year_projected'] = dict((k,v/dayofyear*365) for (k,v) in summary_dict['this_year'].items())
